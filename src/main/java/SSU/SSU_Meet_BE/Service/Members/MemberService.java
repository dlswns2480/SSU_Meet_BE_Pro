...
//    }
        ...
        if (jsoupService.crawling(signInDto)) {      // 유세인트 조회 성공하면
            ...
            if (findUser.isPresent()) { // DB에 있으면
                if (findUser.get().getFirstRegisterCheck().equals(1)) { // 첫 등록 했을 경우
                    log.info("----");
                    return tokenCheck(request, findUser.get(), "RegisteredUser"); // 토큰 체크
                } else { // 첫 등록 안 했을 경우
                    return tokenCheck(request, findUser.get(), "RequiredFirstRegistration"); // 토큰 체크
                }
            } else { // DB에 없으면 Member save 후 개인정보 등록
                Member member = Member.builder().studentNumber(signInDto.getStudentNumber()).build();
                memberRepository.save(member);
                return tokenCheck(request, member, "RequiredFirstRegistration"); // 토큰 체크
            }
        } else {                   // 유세인트 조회 실패
    ...
    // 공통 - 토큰 체크, 생성 까지
    private ApiResponse tokenCheck(HttpServletRequest request, Member findUser, String message) {
    ...
    public ApiResponse mainPage(HttpServletRequest request, Pageable pageable) {
        ...
        if (jwtAuthenticationFilter.parseBearerToken(request) == null) {  // 토큰 없이 메인에 접속한 경우
            Page<StickyNote> allStickyNoteList;
            MainAllDto mainAllDto = new MainAllDto();
            MainAllPageZeroNoLoginDto mainAllPageZeroNoLoginDto = new MainAllPageZeroNoLoginDto();

            allStickyNoteList = pagingRepository.findByAllGender(pageable);
            mainAllPageZeroNoLoginDto.addAllStickyCount(allStickyNoteList.getTotalElements());
            ...
            for (StickyNote mainStickyNote : allStickyNoteList) {
                ...
                MainIdDto mainIdDto = MainIdDto.builder()
                        ...
                        .build();
                mainAllPageZeroNoLoginDto.addMainIdDto(mainIdDto);
                mainAllDto.addMainIdDto(mainIdDto);

            }

            if (pageable.getPageNumber() == 0) { // 첫번 째 페이지면
                return ApiResponse.success("SuccessFirstMainPageAccessNoLogin", mainAllPageZeroNoLoginDto);
            } else {
                return ApiResponse.success("SuccessMainPageUpToOneNoLogin", mainAllDto);
            }
        }

        else {              // 토큰 있고 메인에 접속한 경우
            try {
                Optional<Member> member = getMemberFromToken(request);
                if (member.isPresent()) {
                    Page<StickyNote> allStickyNoteList;
